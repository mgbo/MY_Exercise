# https://github.com/bennuttall/uno/blob/master/uno.py
# http://pythonz.net/references/named/property/
# https://ru.wikipedia.org/wiki/Уно
# TODO:
# 0) choose card by its abbreviation as in print hand.
# 1) say UNO
# 2) ? end of bank desk - shuffle played cards into the bank deck expect the top one.
# 3) count score
# 4) * different text output color ]
# 5) resort card in hand by color (rgby), numbers, special, wilds
# 6) localization
# REGRESSION:
# 1) +2
# 2) skip
...
# -----
# TESTS:
# 1) read initial state from json file (play first playable card in hand), set TEST mode (to eliminate 7 started cards in hands)
# 2) record the party to json log file.
# 3) replay recorded party from a json log file.
# -----
# Подумать над клиент-серверной версией игры, продумать протокол обмена клиентов и сервера.
# Компиляция в исполняемый модуль.
...
if __name__ == '__main__':
    ...
    def mytest():
        ...
        #print (a == b) # True проверять карты совпадают или нет
